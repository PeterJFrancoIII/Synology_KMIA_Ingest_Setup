...
Tab 2 — Forecast vs observed: every release in 0–36h anchor window (golden-master style).
Tab 3 — Accuracy explorer: day/week/month buckets with stability toggles.
...
def build_release_window(df: pd.DataFrame) -> dict:
    """Per-release points and daily aggregates for 0–36h golden-master view."""
    releases = []
    for _, r in df.iterrows():
        lead = float(r["hours_before_target_anchor"])
        releases.append(
            {
                "date": r["target_date_et"],
                "f": round(float(r["forecast_temp_f"]), 2),
                "lead": round(lead, 1),
                "stab": r["forecast_stability"],
            }
        )
    daily = []
    for date, g in df.groupby("target_date_et", sort=True):
        daily.append(
            {
                "date": date,
                "lo": round(float(g["forecast_temp_f"].min()), 2),
                "hi": round(float(g["forecast_temp_f"].max()), 2),
                "mean": round(float(g["forecast_temp_f"].mean()), 2),
                "median": round(float(g["forecast_temp_f"].median()), 2),
                "obs": round(float(g["observed_max_f"].iloc[0]), 2),
                "stab": g["forecast_stability"].iloc[0],
                "n": int(len(g)),
            }
        )
    return {"releases": releases, "daily": daily, "lead_max": 36.0}
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    #chart-timeline, #chart-releases, #chart-accuracy {{ width: 100%; min-height: 480px; }}
    .stats {{
      display: flex;
      flex-wrap: wrap;
      gap: 1rem;
...
<body>
  ...
  <nav class="tabs">
    ...
    <button class="tab" data-tab="releases">Forecast vs observed (0–36h)</button>
  ...
  <section id="panel-releases" class="panel">
    <div class="controls">
      <label>From <input type="date" id="rel-start"/></label>
      <label>To <input type="date" id="rel-end"/></label>
      <label>Min lead (h) <input type="number" id="rel-lead-min" min="0" max="36" step="1" value="0" style="width:4rem"/></label>
      <label>Max lead (h) <input type="number" id="rel-lead-max" min="0" max="36" step="1" value="36" style="width:4rem"/></label>
      <label><input type="checkbox" id="rel-show-releases" checked/> Individual releases</label>
      <label><input type="checkbox" id="rel-color-stab" checked/> Color releases by stability</label>
    </div>
    <div id="chart-releases"></div>
    <div class="stats" id="rel-stats"></div>
  </section>
  ...
  <script>
    ...
    const releaseWindow = PAYLOAD.release_window;
    ...
    function setTab(name) {{
      ...
      else if (name === 'releases') drawReleases();
    ...
    function drawReleases() {{
      ...
      const leadMin = parseFloat(document.getElementById('rel-lead-min').value) || 0;
      const leadMax = parseFloat(document.getElementById('rel-lead-max').value) || 36;
      const showReleases = document.getElementById('rel-show-releases').checked;
      const colorStab = document.getElementById('rel-color-stab').checked;

      const daily = filterByDate(releaseWindow.daily, start, end);
      const dates = daily.map(d => d.date);
      const lo = daily.map(d => d.lo);
      const hi = daily.map(d => d.hi);
      const mean = daily.map(d => d.mean);
      const median = daily.map(d => d.median);
      const obs = daily.map(d => d.obs);
      ...
      const traces = [
        ...
        {{
          x: dates, y: lo, mode: 'lines', fill: 'tonexty',
          fillcolor: 'rgba(76,154,255,0.18)', line: {{ width: 0 }},
          name: 'Forecast range (releases in window)', hoverinfo: 'skip',
        }},
        {{
          x: dates, y: mean, mode: 'markers', name: 'Mean forecast',
          marker: {{ color: '#ffb74d', size: 8, symbol: 'diamond' }},
          hovertemplate: '%{{x}}<br>mean %{{y:.1f}}°F<extra></extra>',
        }},
        {{
          x: dates, y: median, mode: 'markers+lines', name: 'Median forecast',
          line: {{ color: '#66bb6a', width: 1.5 }},
          marker: {{ color: '#66bb6a', size: 6 }},
          hovertemplate: '%{{x}}<br>median %{{y:.1f}}°F<extra></extra>',
        }},
        {{
          x: dates, y: obs, mode: 'markers', name: 'Observed daily max',
          marker: {{ color: '#ef5350', size: 11, symbol: 'x', line: {{ width: 2.5, color: '#ef5350' }} }},
          hovertemplate: '%{{x}}<br>observed %{{y:.1f}}°F<extra></extra>',
        }},
      ];

      let rels = releaseWindow.releases.filter(r =>
        (!start || r.date >= start) && (!end || r.date <= end) &&
        r.lead >= leadMin && r.lead <= leadMax
      );

      if (showReleases) {{
        if (colorStab) {{
          ['STABLE', 'MIXED', 'UNSTABLE'].forEach(stab => {{
            const sub = rels.filter(r => r.stab === stab);
            if (!sub.length) return;
            traces.push({{
              x: sub.map(r => r.date), y: sub.map(r => r.f),
              mode: 'markers', name: 'Release · ' + stab,
              marker: {{ color: STAB_COLORS[stab], size: 5, opacity: 0.85 }},
              hovertemplate: '%{{x}}<br>%{{y:.1f}}°F · ' + stab + '<br>lead %{{customdata}}h<extra></extra>',
              customdata: sub.map(r => r.lead),
            }});
          }});
        }} else {{
          traces.push({{
            x: rels.map(r => r.date), y: rels.map(r => r.f),
            mode: 'markers', name: 'Individual forecast release',
            marker: {{ color: '#4c9aff', size: 5, opacity: 0.75 }},
            hovertemplate: '%{{x}}<br>%{{y:.1f}}°F · lead %{{customdata}}h<extra></extra>',
            customdata: rels.map(r => r.lead),
          }});
        }}
      }}
      ...
      const layout = {{
        ...layoutBase,
        title: 'Historical NDFD releases vs observed max (0–36h before 4 PM ET)',
        margin: {{ l: 56, r: 24, t: 48, b: 80 }},
        xaxis: dateXAxis('Target date (ET)', true),
        yaxis: {{ title: 'Temperature (°F)', gridcolor: '#2d3a4f' }},
      }};
      Plotly.react('chart-releases', traces, layout, {{ responsive: true }});

      document.getElementById('rel-stats').innerHTML =
        `<span><strong>${{daily.length}}</strong> target days</span>` +
        `<span><strong>${{rels.length}}</strong> releases in window</span>` +
        `<span>Lead filter <strong>${{leadMin}}–${{leadMax}}h</strong> before 4 PM ET anchor</span>`;
    }}
    ...
    function initDates() {{
      ...
      ['tl-start','tl-end','acc-start','acc-end','rel-start','rel-end'].forEach(id => {{
        const el = document.getElementById(id);
        el.min = calMin;
        el.max = calMax;
      }});
      ...
      document.getElementById('rel-start').value = dataMin;
      document.getElementById('rel-end').value = dataMax;
    ...
    ['rel-start','rel-end','rel-lead-min','rel-lead-max','rel-show-releases','rel-color-stab'].forEach(id =>
      document.getElementById(id).addEventListener('change', drawReleases));
    ['rel-lead-min','rel-lead-max'].forEach(id =>
      document.getElementById(id).addEventListener('input', drawReleases));
...
def main() -> int:
    ...
    payload = {
        ...
        "release_window": build_release_window(df),
        "buckets": {
