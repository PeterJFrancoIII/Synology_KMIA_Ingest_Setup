...
│ Legion5 or Mac: chart_kmia_year_stability_wind.py / long_period charts      │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
### Phase 3 — CHARTS (optional, not on critical path for tables)

**Where:** Legion5 (has data) or Mac (golden master reference)  
**Script:** `chart_kmia_year_stability_wind.py` per year after `32_finalize_year.sh`  
**Time:** ~2–5 min/year (matplotlib; not benchmarked in depth)
